...
int setup_binding(Binding *binding)
{
	...
	if (ret < 0) {
		log_error("Error binding source to target");
		exit(EXIT_FAILURE);
	}
...
{
	// check if videocapture is already running
	char vbuf[12] = "";
	...
	int v;
	for (v = 0; v < sizeof(vbuf) && vbuf[v] != '\0'; v++) {
		if (vbuf[v] == ' ') {
			printf("videocapture already running!\n");
			return -1;
		}
	}
	...
	// Inialize image sensor and audio	
	...
	// Load json data into structures
	load_configuration(json, camera_config);
	// Start thread that update on-screen display data
	start_osd_update_threads(camera_config);
	// Initialize framesources
