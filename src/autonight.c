...
#define GAIN "ISP total gain : "

const char *nightModeCmd = "/usr/bin/nightmode.sh";
const char *dayChangeCmd = NULL; // default: on
const char *nightChangeCmd = NULL; // default: off
int verbose = 0;
...
int eq1_user_exposure = 1260000;
int eq2_user_exposure = 900000;
...
void usage();
int readIspInfo(unsigned int *exposure, unsigned int *iridix, unsigned int *colortemp, unsigned int *gain);
...
int main(int argc, char *argv[]) {
	...
	while (true) {
		...
		if (!ret) {
			...
			nsleep(delayBetweenReads*MICROSEC);
			continue;
		}
...
void usage() {
	...
	printf("-d <int>	Delay (in milliseconds) between averaged reads (default: %d)\n", delayBetweenReads);
...
int readIspInfo(unsigned int *exposure, unsigned int *iridix, unsigned int *colortemp, unsigned int *gain) {
	...
	buf[99] = 0;
	if (!exposure || !iridix || !colortemp || !gain) {
	...
	while ((bytes_read = read(fd, &buf[bytes_in_buf], bytes_to_read)) > 0 || bytes_in_buf > 0) {
		...
		if(strncmp(buf, GAIN, sizeof(GAIN)-1) == 0){
			*gain = atoi(&buf[sizeof(GAIN)-1]);
			ret++;
		}
		else if(strncmp(buf, EXPOSURE, sizeof(EXPOSURE)-1) == 0){
...
int software_method(void) {
	...
	int exposure, iridix, colortemp, gain;
	...
	int swBlock = 0;
	long int elapsedSeconds = 0, elapsedSeconds2 = 0;
	struct timeval timeLastSw, timeUnlock, timeNow, timeRes;
	struct timeval fiveMinutes = { 5*60, 0L };
	gettimeofday(&timeLastSw, NULL);
	timersub(&timeLastSw, &fiveMinutes, &timeRes);
	timeLastSw = timeRes;
	gettimeofday(&timeUnlock, NULL);
	
	
	while (1) {
		if (readIspInfo(&exposure, &iridix, &colortemp, &gain) == 4) {
			if(verbose >= 2) printf("(gain: %d, exposure: %d, iridix: %d, colortemp: %d)\n", gain, exposure, iridix, colortemp);
			...
			if (exposure > 1260000 && gain >= 8192 /*&& iridix >= 13*/ && colortemp >= 3700) {
				if (verbose >= 1) printf("night: ");
				
				if (nightModeEnabled == false && swBlock == 0) {
					if (verbose >= 1) printf("Switching to night mode... ");
					nightModeEnabled = true;
					updateNightMode();
					
					gettimeofday(&timeNow, NULL);
					timersub(&timeNow, &timeLastSw, &timeRes);
					elapsedSeconds = (long int)timeRes.tv_sec + ((long int)timeRes.tv_usec / MICROSEC);
					timeLastSw = timeNow;
					
					if (elapsedSeconds < 15 && swBlock == 0) {
						swBlock = 1;
						gettimeofday(&timeUnlock, NULL);
					}
				}
			...
			else if ((exposure < 980000 && gain < 6500 && iridix >= 5 && colortemp <= 2800) || 
					 (exposure < 700000 && gain <= 550 && iridix >= 20)) {
				if (verbose >= 1) printf("day: ");
				
				if (nightModeEnabled == true && swBlock == 0) {
					if (verbose >= 1) printf("Switching to day mode... ");
					nightModeEnabled = false;
					updateNightMode();
			...
			if (swBlock != 0) {				
				...
				timersub(&timeNow, &timeUnlock, &timeRes);
				elapsedSeconds2 = (long int)timeRes.tv_sec + ((long int)timeRes.tv_usec / MICROSEC);
				
				printf("> time to unlock, not trying: %d/30\n", elapsedSeconds); // resets every time it tries to change to the other mode, if it reaches x (ex: 20sec) without trying to change to the other mode, it unlocks and then it allows to change
				printf("> time to unlock, force: %d/5min\n", elapsedSeconds2); // if it really wants to change mode, it counts continuously to x (ex: 5min) and then changes
				
				if (elapsedSeconds >= 30 || elapsedSeconds2 >= 60*5) {
					swBlock = 0;
					gettimeofday(&timeUnlock, NULL);
					timersub(&timeLastSw, &fiveMinutes, &timeRes);
					timeLastSw = timeRes;
...
// adapted from: https://stackoverflow.com/a/33412960
// 1s, 1000 ms, 1000000 us, 1000000000 ns (NANOSEC)
int nsleep(unsigned long nanoseconds)
{
	struct timespec req, rem;

	if (nanoseconds > NANOSEC-1) {
		req.tv_sec = (int)(nanoseconds / NANOSEC);
		req.tv_nsec = (nanoseconds - ((long)req.tv_sec * NANOSEC));
	}   
	else {
		req.tv_sec = 0;
